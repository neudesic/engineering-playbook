...
Microsoft runs on trust. Our customers trust NEE to adhere to the highest standards when handling their data.
...
Developers working on NEE projects should implement best practices and guidance on handling data throughout the project phases. This page is not meant to suggest how customers should handle data in their environment. **It does not override**:
...
## Handling Data in NEE Engagements
...
Developers working on NEE projects will work with our customers to define the data needed for each engagement.

If there is a need to access production data,
NEE needs to review the need with their lead and work with the customer to put audits in place verifying what data was accessed.
...
Customers should provide NEE with a copy of the requested data in a location managed by the customer.
...
NEE should notify the customer when they are done with the data and suggest the customer destroy copies of the data if they are no longer needed.
